...
pub fn solver(mat_size: usize, radial_nodes: Vec<f64>) {
    ...
    let mut sigma: Array1<f64> = Array1::zeros(2);
    ...
    for (_, &t_step) in time_span.iter().enumerate() {
        ...
        if t_step <= input_parameters::T_LOAD_END {
            ...
            load =
                <u32 as Into<f64>>::into(input_parameters::P_MAX * input_parameters::INNER_RADIUS)
                    * _load_scale;
        } else {
            load =
                <u32 as Into<f64>>::into(input_parameters::P_MAX * input_parameters::INNER_RADIUS);
        ...
        while newton_iterations <= input_parameters::MAX_ITERATIONS {
            ...
            for element in 0..(mat_size) {
                let mut r1 = radial_nodes[element];
                let mut r2 = radial_nodes[element + 1];
                ...
                let mut prev_sigma_ov = sigma_ov.clone();
                let mut jacobian = (r2 - r1) / 2.0;
                let xi: f64 = input_parameters::N_INTEGRATION_POINTS.into();
                ...
                .unwrap();
                let strain = b_mat.dot(&e_displacement);
                let delta_strain = b_mat.dot(&delta_u_element);
                // Material Routine
                let mut tangent_stiffness: Array2<f64> = Array2::zeros((2, 2));
                // Element stiffness
